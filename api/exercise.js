...
router
  ...
  .get(function (req, res) {
    ...
    // }
    // if (!keyword) {
    //   res.status(400).json({ message: 'no keyword' });
    //   return;
    // }
    ...
    //   clue['where']['name'] = { [Op.like]: '%' + keyword + '%' };
    // } else if (type === 'search1') {
    //   clue['where']['machine'] = { [Op.like]: '%' + keyword + '%' };
  ...
  .put(function (req, res) {
    ...
    if (type === 'set_default') {
      data.forEach((d) => {
        Exercise.update({ is_default: d[1] }, { where: { exercise_no: d[0] } })
          .then(() => {
            console.error('okay');
          })
          .catch((err) => {
